...
export async function getScannedPageUrlsForDomain(domain) {
  if (!domain) return { urls: new Set(), scanCount: 0, pageCount: 0 };
  const scans = await getAllScans();
  const target = String(domain).toLowerCase();
  const matching = scans.filter(
    (s) =>
      (s.domain || '').toLowerCase() === target &&
      (s.status === 'completed' || s.status === 'cancelled' || s.status === 'running')
  );
  const urls = new Set();
  for (const scan of matching) {
    const pages = await getPagesByScan(scan.id);
    for (const page of pages) {
      const norm = normalizePageUrl(page.url);
      if (norm) urls.add(norm);
    }
  }
  return { urls, scanCount: matching.length, pageCount: urls.size };
}
